...
namespace GC {
    ...
    public:
        ...
        const bool isLeaf() {std::holds_alternative<Data>(contents);}
    ...
    public:
        ...
        std::string to_string() const {
            ...
            auto dumpPair = [&](Symbol key, BitVector value) {ss<<"["<<key<<"]->"; dumpBitVector(value);};
            bool isFirst = true;
    ...
    public:
        ...
        Encoder getEncoder(const std::function<void(bool)>& handler) {
            return Encoder(encoderMap, handler);
        }

        Decoder getDecoder(const std::function<void(const Symbol&)> handler, const std::function<bool(void)> requester) {
        ...
        std::string to_string() {
            std::stringstream ss;
            ss<<"Huffman Coder: {";
            auto showBitVector = [&](const std::vector<bool> bv) {
                for (bool b: bv) {
                    ss << b;
                }
            };

            for (const auto&[symbol, bv] : encoderMap) {
                ss<<symbol<<" -> ";
                showBitVector(bv);
                ss<<", ";
            }
